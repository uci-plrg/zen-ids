...
CREATE TRIGGER evo_update_wp_commentmeta AFTER UPDATE ON wp_commentmeta
  ...
  BEGIN
    IF NEW.comment_id != OLD.comment_id THEN
      REPLACE INTO opmon_evolution VALUES ('wp_commentmeta', 'comment_id', NEW.meta_id, connection_id());
    END IF;
    IF NEW.meta_key != OLD.meta_key THEN
      REPLACE INTO opmon_evolution VALUES ('wp_commentmeta', 'meta_key', NEW.meta_id, connection_id());
    END IF;
    IF NEW.meta_value != OLD.meta_value THEN
      REPLACE INTO opmon_evolution VALUES ('wp_commentmeta', 'meta_value', NEW.meta_id, connection_id());
    END IF;
  END $$
