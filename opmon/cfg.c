...
{
  routine_cfg_t *cfg = PROCESS_ALLOC(routine_cfg_t);
                               ...
                               ushort line_number, uint index)
{
  ...
  while (index >= cfg->opcodes.size) {
    cfg_opcode = PROCESS_ALLOC(cfg_opcode_t);
...
{
  cfg_opcode_edge_t *cfg_edge = PROCESS_ALLOC(cfg_opcode_edge_t);
                          ...
                          user_level_t user_level)
{
  ...
  cfg_routine_edge_entry_t *cfg_entry = PROCESS_ALLOC(cfg_routine_edge_entry_t);
